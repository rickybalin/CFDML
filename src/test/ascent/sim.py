...
def gen_hex_mesh(num_elms, rank):
    ...
    n = np.empty([8])
    # PHASTA vertex ordering (switch loops to be i->j->k)
    #n[0] = 0
    #n[1] = num_vtx+1
    #n[2] = num_vtx+2
    #n[3] = 1
    #n[4] = num_vtx*num_vtx+1
    #n[7] = num_vtx*num_vtx+2
    #n[6] = num_vtx*(num_vtx+1)+2
    #n[5] = num_vtx*(num_vtx+1)+1
    # VTK vertex ordering (loops are k->j->i)
    n[0] = 0
    n[1] = 1
    n[2] = num_vtx+1
    n[3] = num_vtx
    ...
    for k in range(num_elms):
        for j in range(num_elms):
            for i in range(num_elms):
                #tmp1 = k+j*num_elms+i*num_elms*num_elms # PHASTA ordering
